...
import { handler } from "./index.server";
  ...
  return {
    ...
    headers: {
      ...
      "x-amzn-lambda-proxy-auth":
        "HmacSHA256, SignedHeaders=x-amzn-lambda-forwarded-client-ip;x-amzn-lambda-forwar" +
      ...
      "x-amzn-lambda-forwarded-host":
        "4ruyrzea3inreev7qr5r3avd7m0ukmzf.lambda-url.us-east-1.on.aws",
      ...
      "x-amzn-trace-id":
        "Self=1-65c08e4d-69f9a85a3e3e0d79149e7615;Root=1-65c08e4d-0f1cf053229cd24b5f68dda" +
        "7",
    ...
    requestContext: {
      ...
      domainName:
        "4ruyrzea3inreev7qr5r3avd7m0ukmzf.cell-1-lambda-url.us-east-1.on.aws",
...
app.get("/", async (req, res) => {
  try {
    const response = await handler(createFakeEvent(req.path));
    res.send((response as APIGatewayProxyStructuredResultV2).body);
  } catch (error) {
    console.log({ error }, "/");
    res.send({ status: 500 });
  }
