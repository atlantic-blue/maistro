...
const BusinsessProfileServices = ({
    services
          ...
          {services.length === 0 ? (
            ...
            <div className="space-y-4">
              {services.map((service) => (
                <Card key={service.id} className="bg-white border border-gray-100 rounded-xl p-5 hover:shadow-md transition-all">
                  <div className="flex justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-5 shrink-0">
                      <span className="text-lg font-bold text-gray-900">
                        {typeof service.price === 'number' ? `desde ${formatPrice(service.price)}` : 'Precio a consultar'}
                      </span>
                      <Button className="bg-[#FF3366] hover:bg-[#D94A6A] text-white px-5">Reservar</Button>
                    </div>
                  </div>
                </Card>
              ))}
