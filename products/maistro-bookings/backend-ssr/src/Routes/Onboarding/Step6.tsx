...
const Step6: React.FC<Step1Props> = ({
  ...
  const selectedCategoryServices: Service[] = sampleServices[formData.businessType[0]] || [];

  const handleAddCustomField = () => {
    setCustomServices([...customServices, { name: '', duration: '', price: 0 }]);
  };
  const handleUpdateCustom = (index: number, field: keyof Service, value: string | number) => {
  ...
  const handleSubmitCustomServices = () => {
    const valid = customServices.filter((s) => s.name && s.duration && s.price > 0);
    if (valid.length > 0) {
      updateFormData({
  ...
  return (
    ...
    >
      <div>
        ...
        <h1 className="text-4xl font-bold text-gray-900 mb-4">{resourceStrings.step6.title}</h1>
        <p className="text-gray-600 mb-8">{resourceStrings.step6.description}</p>

        <div className="space-y-4 max-w-2xl mx-auto">
          {[...new Set([...selectedCategoryServices, ...formData.services])].map(
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      ...
                      {isSelected && <Check className="w-5 h-5 text-[#FF3366] ml-4 inline-block" />}
                    </div>
                  </div>
                </div>
              );
          ...
          {customServices.length > 0 && (
            <div className="space-y-4">
              {customServices.map((service, index) => (
                <div key={index} className="p-4 border rounded-xl bg-gray-50 relative">
                  ...
                  <input
                    ...
                    value={service.name}
                    onChange={(e) => handleUpdateCustom(index, 'name', e.target.value)}
                  />
                  <input
                    className="w-full mb-2 p-3 border rounded-xl"
                    placeholder="Duration (e.g. 45 mins)"
                    value={service.duration}
                    onChange={(e) => handleUpdateCustom(index, 'duration', e.target.value)}
                  />
                  <input
                    type="number"
                    className="w-full mb-2 p-3 border rounded-xl"
                    ...
                    value={service.price || ''}
                    onChange={(e) =>
                      handleUpdateCustom(index, 'price', parseFloat(e.target.value) || 0)
                    }
                  />
                </div>
              ))}
              ...
              >
                {resourceStrings.step6.saveCustomService || 'Add All Custom Services'}
              </button>
            </div>
          )}
