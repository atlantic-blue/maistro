  ...
  return (
    ...
    >
      <div>
        ...
        <h1 className="text-4xl font-bold text-gray-900 mb-4">{resourceStrings.step2.title}</h1>
        <p className="text-gray-600 mb-12">{resourceStrings.step2.description}</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {businessCategories.map((category: BusinessCategory) => {
            const IconComponent = category.icon;
            const isSelected: boolean = formData.businessType.includes(category.id);
            ...
            return (
              <button
                key={category.id}
                onClick={() => updateFormData({ businessType: updatedBusinessType })}
