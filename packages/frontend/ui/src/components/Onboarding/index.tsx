...
const Onboarding: React.FC<OnboardingProps> = (props): React.ReactNode => {
  ...
  const processOnboardUser = async () => {
    ...
    try {
      await onboardUser(props.onboardingUrl, props.token, formData);
      setIsSubmitted(true);
    } catch (error) {
      // TODO
      console.log({ error });
  ...
  useEffect(() => {
    if (props.isLoading || !props.isAuthenticated || currentStep !== 7) {
      return;
    }
...
export { Onboarding as MaistroOnboarding };
