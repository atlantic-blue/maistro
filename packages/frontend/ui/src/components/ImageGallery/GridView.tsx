...
function AestheticPlaceholder({ label = 'Maistro', full = false }: { label?: string; full?: boolean }) {
return (
<div className={`relative ${full ? 'w-full h-full' : 'w-full h-64 sm:h-80 md:h-[60vh]'} overflow-hidden rounded-2xl`}>
...
function altFor(image: MaistroImage) { return `Imagen ${image.ImageId}`; }

async function copyToClipboard(text: string) {
try { await navigator.clipboard.writeText(text); } catch {}
}

export function GridView({ images, onOpen, onDelete, busyIds }:{ images: MaistroImage[]; onOpen: (i:number)=>void; onDelete: (id:string)=>void; busyIds: Record<string, boolean>}) {

return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
    {images.map((image, i) => (
    <div key={image.ImageId} className="group relative overflow-hidden rounded-2xl bg-white border border-neutral-200 shadow-sm">
    <div className="aspect-square w-full overflow-hidden bg-neutral-100">
    {pickThumbUrl(image) ? <img
    ...
    <Button size="1" variant="solid" onClick={() => window.open(image.Urls!.Optimized!, '_blank')}>
    ...
    <Button size="1" variant="soft" onClick={() => copyToClipboard(image.Urls!.Optimized!)}>
    <Copy size={14} />
    </Button>
    </Tooltip>
    )}
    ...
    <Button size="1" color="red" variant="soft" onClick={() => onDelete(image.ImageId)} disabled={!!busyIds[image.ImageId]}>
    <Trash2 size={14} />
    </Button>
    </Tooltip>
    </Flex>
