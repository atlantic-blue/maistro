...
export function MaistroImageUploader({
  ...
  const handleUpload = async (id: string) => {
    ...
    if (['DONE', 'UPLOADING', 'PROCESSING', 'ERROR', 'CANCELED'].includes(item.state)) {
      console.log(`Aborting Item ${item.id} | State: ${item.state}`);
      return;
    }

    setItems((prev) => {
      return updateItem(prev, id, { state: 'SIGNING', progress: 0, message: '' });
    });
    ...
    try {
      ...
      xhr.upload.onprogress = (evt) => {
        ...
        setItems((current) =>
          updateItem(current, id, { progress: percentage, speedBps: speed, etaSec: eta })
        );
      };
      ...
      setItems((cur) =>
        updateItem(cur, id, { state: 'ERROR', message: error.message || 'Upload failed' })
      );
    }
  };
  ...
  const totalPct = React.useMemo(() => {
    ...
    const active = items.filter((item) =>
      ['UPLOADING', 'PROCESSING', 'SIGNING'].includes(item.state)
    );
    if (active.length === 0) {
      return 0;
    }
    ...
    const sum = active.reduce(
      (acc, it) => acc + (it.state === 'PROCESSING' ? 100 : it.progress),
      0
    );
    ...
    <Theme appearance="light" accentColor="crimson" grayColor="sand" radius="large" scaling="100%">
      ...
      >
        ...
        <Flex className="py-6" direction="column">
          ...
          {items.length === 0 && (
            <Text className="py-3" color="gray">
              No hay archivos aún. Usa “Seleccionar archivos” o arrastra aquí.
            </Text>
          )}
        </Flex>
      </Card>
